...
variable {n : ℕ}
@[simp]
def α₁ (S : (PlusU1 n).AnomalyFreeQuad) : ℚ := - 3 * cubeTriLin (S.val, S.val, (BL n).val)

@[simp]
def α₂ (S : (PlusU1 n).AnomalyFreeQuad) : ℚ := accCube S.val

lemma cube_α₁_α₂_zero (S : (PlusU1 n).AnomalyFreeQuad) (a b : ℚ) (h1 : α₁ S = 0) (h2 : α₂ S = 0) :
    accCube (BL.addQuad S a b).val = 0 := by
  erw [BL.add_AFL_cube]
  simp_all

lemma α₂_AF (S : (PlusU1 n).AnomalyFree) : α₂ S.toAnomalyFreeQuad = 0 := S.2

lemma BL_add_α₁_α₂_cube (S : (PlusU1 n).AnomalyFreeQuad) :
    accCube (BL.addQuad S (α₁ S) (α₂ S)).val = 0 := by
  erw [BL.add_AFL_cube]
  field_simp
  ring_nf

lemma BL_add_α₁_α₂_AF (S : (PlusU1 n).AnomalyFree) :
    BL.addQuad S.1 (α₁ S.1) (α₂ S.1) = (α₁ S.1) • S.1 := by
  rw [α₂_AF, BL.addQuad_zero]

def generic (S : (PlusU1 n).AnomalyFreeQuad) : (PlusU1 n).AnomalyFree :=
  quadToAF (BL.addQuad S (α₁ S) (α₂ S)) (BL_add_α₁_α₂_cube S)

lemma generic_on_AF (S : (PlusU1 n).AnomalyFree) : generic S.1 = (α₁ S.1) • S := by
  apply ACCSystem.AnomalyFree.ext
  change (BL.addQuad S.1 (α₁ S.1) (α₂ S.1)).val = _
  rw [BL_add_α₁_α₂_AF]
  rfl

lemma generic_on_AF_α₁_ne_zero (S : (PlusU1 n).AnomalyFree) (h : α₁ S.1 ≠ 0) :
    (α₁ S.1)⁻¹ • generic S.1 = S := by
  rw [generic_on_AF, smul_smul, inv_mul_cancel h, one_smul]

def special (S : (PlusU1 n).AnomalyFreeQuad) (a b : ℚ) (h1 : α₁ S = 0) (h2 : α₂ S = 0) :
    (PlusU1 n).AnomalyFree :=
  quadToAF (BL.addQuad S a b) (cube_α₁_α₂_zero S a b h1 h2)

lemma special_on_AF (S : (PlusU1 n).AnomalyFree)  (h1 : α₁ S.1 = 0) :
...
open QuadSolToSol
def quadSolToSol {n : ℕ} : (PlusU1 n).AnomalyFreeQuad × ℚ × ℚ → (PlusU1 n).AnomalyFree := fun S =>
  if h1 : α₁ S.1 = 0 ∧  α₂ S.1 = 0 then
    special S.1 S.2.1 S.2.2 h1.1 h1.2
  else
    S.2.1 • generic S.1

def quadSolToSolInv {n : ℕ} : (PlusU1 n).AnomalyFree → (PlusU1 n).AnomalyFreeQuad × ℚ × ℚ :=
    fun S =>
  if α₁ S.1 = 0 then
    (S.1, 1, 0)
  else
...
lemma quadSolToSolInv_1 (S : (PlusU1 n).AnomalyFree)  :
    (quadSolToSolInv S).1 = S.1 := by
  simp [quadSolToSolInv]
  split
  rfl
  rfl
lemma quadSolToSolInv_α₁_α₂_zero (S : (PlusU1 n).AnomalyFree) (h : α₁ S.1 = 0) :
    α₁ (quadSolToSolInv S).1 = 0 ∧ α₂ (quadSolToSolInv S).1 = 0 := by
  rw [quadSolToSolInv_1, α₂_AF S, h]
  simp

lemma quadSolToSolInv_α₁_α₂_neq_zero (S : (PlusU1 n).AnomalyFree) (h : α₁ S.1 ≠  0) :
   ¬  (α₁ (quadSolToSolInv S).1 = 0 ∧ α₂ (quadSolToSolInv S).1 = 0) := by
  rw [not_and, quadSolToSolInv_1, α₂_AF S]
  intro hn
  simp_all

lemma quadSolToSolInv_special (S : (PlusU1 n).AnomalyFree) (h : α₁ S.1 = 0) :
...
lemma quadSolToSolInv_generic (S : (PlusU1 n).AnomalyFree) (h : α₁ S.1 ≠ 0) :
